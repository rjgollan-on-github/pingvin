...
transform_pq_vel_to_local_frame :: proc (P: ^[Primitive_Quantities]complex128, n, t1, t2: Vector3) {
    vel := Vector3{P[.xvel], P[.yvel], P[.zvel]}
    P[.xvel] = dot(vel, n)
    P[.yvel] = dot(vel, t1)
    P[.zvel] = dot(vel, t2)
    return
...
transform_flux_to_global_frame :: proc (F: ^[Conserved_Quantities]complex128, n, t1, t2: Vector3) {
    Fmom := Vector3{F[.xmom], F[.ymom], F[.zmom]}
    F[.xmom] = Fmom.x*n.x + Fmom.y*t1.x + Fmom.z*t2.x
    F[.ymom] = Fmom.x*n.y + Fmom.y*t1.y + Fmom.z*t2.y
    F[.zmom] = Fmom.x*n.z + Fmom.y*t1.z + Fmom.z*t2.z
    return
}
